...
class DailyReportService
{
    ...
    public function build(int $shedId, string $reportDate, string $version = 'en'): array
    {
        ...
        // If water is already stored in Liters
        $totalWater = ProductionLog::where('shed_id', $shedId)
            ->where('flock_id', $latestFlock->id)
            ->sum(DB::raw('day_water_consumed + night_water_consumed'));

        $totalWaterL = round($totalWater, 2) . ' L'; // no division

        return $version === 'en'
            ? $this->formatEnglish($shed, $latestFlock, $data, [$totalWaterL])
            : $this->formatUrdu($shed, $latestFlock, $data);
    }

    private function formatEnglish($shed, $flock, $data, $summary): array
    {
        return [
            'shed' => $shed->name,
            'flock' => $flock->name,
            ...
            '24h_mortality' => $data->day_mortality_count + $data->night_mortality_count,
            'todate_mortality_count' => $data->todate_mortality,
            ...
            '24h_feed_consumed' => round(($data->day_feed_consumed + $data->night_feed_consumed) / 1000, 2) . ' Kg',
            ...
            '24h_water_consumed' => round($data->day_water_consumed + $data->night_water_consumed, 2) . ' L',
            'avg_water_consumed' => round($data->avg_water_consumed, 2) . ' L',
            'total_water_consumed' => $summary[0],
    ...
    private function formatUrdu($shed, $flock, $data): array
    {
        return [
            ...
            'کل اموات تعداد' => ($data->day_mortality_count + $data->night_mortality_count) . '',
            'ابتک اموات تعداد' => $data->todate_mortality . '',
            ...
            'کل خوراک صرف شدہ' => round(($data->day_feed_consumed + $data->night_feed_consumed) / 1000, 2) . ' کلوگرام ',
            ...
            'کل پانی صرف شدہ' => round($data->day_water_consumed + $data->night_water_consumed, 2) . ' لیٹر ',
